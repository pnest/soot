...
public class FieldRenamer extends SceneTransformer implements IJbcoTransform {
  ...
  protected void internalTransform(String phaseName, Map<String, String> options) {
    ...
    for (SootClass applicationClass : Scene.v().getApplicationClasses()) {
      ...
      // TODO remove
      for (SootField field : applicationClass.getFields()) {
        if (field.getName().equals(opaquePredicate)) {
          throw new IllegalStateException(opaquePredicate);
        }
      }
      // String opaquePredicate = "opaquePredicate1" + className;
      // while (oldToNewFieldNames.containsKey(opaquePredicate)) {
      // opaquePredicate += '_';
      // }
      ...
      // opaquePredicate = "opaquePredicate2" + className;
      // while (oldToNewFieldNames.containsKey(opaquePredicate)) {
      // opaquePredicate += '_';
      // }
    ...
    if (isVerbose()) {
      logger.info("\r\tUpdating field references in bytecode");
    }
